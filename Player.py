...
class Gunner(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, character, scale, speed, ammon, grenade, health, last_score, timer):
        ...
        self.death = False
        self.action_type = {"idle": 5, "run": 6, "jump": 1, "death": 6, "hanging":1}  # no. of frames per dict
        self.animation_list = []
        self.amt_index = 0
        self.action = 0

        # player modes - death, idle, jump , run, hanging
        modes = ["Idle", "Run", "Jump", "Death", "hanging"]
        # 0 - Idle, 1-Run, 2-Jump, 3 - Death, 4 - hanging

        self.mode_frame = {"Idle": 5, "Run": 10, "Jump": 9, "Death": 10, "hanging":1}
            ...
            if self.jump:
                ...
                if not self.in_air:  # if on the floor
                    ...
                    if self.jump_count > 1:
                        pygame.draw.rect(screen, "red", self.rect,2,10)
    ...
    def check_death(self, obstacle_group, tanks_group, animated_group):
        ...
        if pygame.sprite.spritecollide(self, obstacle_group, False) or \
                ...
                pygame.sprite.spritecollide(self, animated_group,False) or \
                self.rect.y > 800:
            self.health = 0
