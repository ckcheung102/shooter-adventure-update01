...
from Bullets import Grenade, Obstacle, Tanks, Animated_sprite
...
game_icon = pygame.image.load("Tile/0.png")
...
def load_images(sprite_folder)-> list:
    paths = [item for item in pathlib.Path(sprite_folder).rglob("*.png") if item.is_file()]
    return [pygame.image.load(str(path)).convert_alpha() for path in paths]


animation_frames = {
    "fire": load_images("animated sprites/fire"),
    "water": load_images("animated sprites/water")
        ...
        for y, row in enumerate(graph_data):
            for x, col in enumerate(row):
                ...
                # stationary enemy
                    ...
                    if col == 36:
                        # instance a fire sprite
                        Animated_sprite(x * TILE_SIZE, y * TILE_SIZE, animation_frames["fire"], [animated_group],
                                        "fire")

                    elif col == 18:
                        # instance a water sprite
                        Animated_sprite(x * TILE_SIZE, y * TILE_SIZE, animation_frames["water"], [animated_group],
                                        "water")
                    else:
                        # stationary obstacle
...
def check_player_action():
    ...
    if player.death is False:
        ...
        if player.in_air and player.hanging==False:
            player.revise_action(2)  # 2 - jump

        # hanging
        elif player.in_air and player.hanging:
            player.revise_action(4)
...
def group_update():
    ...
    animated_group.update(screen, screen_scroll)
...
while running:
    ...
    # Run State #
        ...
        for enemy in enemy_group:
            ...
            enemy.check_death(obstacle_group, tanks_group, animated_group)
        ...
        # check alive or not
        player.check_death(obstacle_group, tanks_group, animated_group)
